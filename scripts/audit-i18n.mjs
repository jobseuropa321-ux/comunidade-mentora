...
const pend = rows.filter(r => r.cand > 0 && !r.traduzido);
const feito = rows.filter(r => r.traduzido);
console.log('JÁ USAM useTranslation (' + feito.length + '):');
for (const r of feito) console.log(`  ${String(r.cand).padStart(4)}  ${r.f}`);
console.log('\nAINDA SEM TRADUÇÃO (' + pend.length + ' arquivos, ~' + pend.reduce((s, r) => s + r.cand, 0) + ' candidatos):');
for (const r of pend.slice(0, 30)) console.log(`  ${String(r.cand).padStart(4)}  ${r.f}`);
