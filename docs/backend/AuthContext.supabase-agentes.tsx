 ...
 *  1. rode o SQL do BACKEND_SETUP.md desta seção
 *  2. apague `AuthContext.tsx` (o mock) e renomeie este pra `AuthContext.tsx`
 *
 *  O Chat/Biblioteca só precisam de `user` (e a cota usa `user_roles`, criada
 *  no SQL desta seção). Já `profile`/`updateProfile` dependem da tabela
 *  `profiles` — ela vem do pacote de PERFIL; se seu app não a tiver, os fetches
 *  falham silenciosamente (profile fica null) sem afetar o chat. Pode remover
 *  fetchProfile/updateProfile se não for usá-los.
 * ═══════════════════════════════════════════════════════════════════════ */
